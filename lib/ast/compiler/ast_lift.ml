...
let make_data eir offset_to_label =
  ...
  let heap_entry =
    Program.Data.{ label = Eir.const_heap_start_label; type_ = Heap }
  in
  if not @@ List.is_empty (Eir.data eir) then (
