(* open Core
   ...
   let lift_imm_or_reg (imm_or_reg : Insn.Imm_or_reg.t) : Expression.t =
     ...
     | Int x -> Const x
     | Register x -> Var (Eir.Register.to_string x)
     | Label x -> Label x
   ...
   let lift_insn (insn : Insn.t) : Statement.t =
     ...
     | Mov { dst; src } ->
         ...
         let dst = Statement.Assignment.Var (Eir.Register.to_string dst) in

         Assign { dst; src }
     | Add { dst; src } ->
         let src = lift_imm_or_reg src in
         Assign { dst = lift_dst_reg dst; src = Add [ Register dst; src ] }
     | Sub { dst; src } ->
         let src = lift_imm_or_reg src in
         Assign { dst = lift_dst_reg dst; src = Sub (Register dst, src) }
     | Load { dst; src } ->
         let src = lift_imm_or_reg src in
         Assign { dst = lift_dst_reg dst; src = Memory src }
     | Store { src; dst } ->
         let dst = lift_imm_or_reg dst in
         Assign { dst = Memory dst; src = Register src }
     | Putc src -> Putc (lift_imm_or_reg src)
     | Getc dst -> Assign { dst = lift_dst_reg dst; src = Getc }
     | Exit -> Exit
     ...
     | Set ({ args; _ } as cond) ->
         ...
         Assign { dst = lift_dst_reg args.dst; src = Set cond }
     | Dump -> Nop
