...
let optimizer = Ast_expression_optimizer.create ()
let print exp = print_s [%sexp (exp : Ast.Expression.t)]
let optimize exp = Ast_expression_optimizer.optimize optimizer exp |> fst
...
let%expect_test "memory address is simplified" =
  optimize (Var (Memory (Sub (Const 1, Const 1)))) |> print;
...
let%expect_test "add combines constants" =
  optimize (Add [ Const 1; Var (Memory (Const 0)); Const 2 ]) |> print;
...
let%expect_test "add removes const 0" =
  optimize (Add [ Const 0; Var (Register A) ]) |> print;
  [%expect {| (Var (Register A)) |}]
...
let%expect_test "sub is 0 when args are same" =
  optimize (Sub (Var (Memory (Const 0)), Var (Memory (Const 0)))) |> print;
  [%expect {| (Const 0) |}]
...
let%expect_test "sub simplifies to add for constants" =
  optimize (Sub (Var (Register A), Const 1)) |> print;
    ...
    (If
       {
         ...
         right = Var (Memory (Const 0));
       })
  |> print;
  [%expect
    {| (If ((cmp Eq) (left (Const 0)) (right (Var (Memory (Const 0)))))) |}];
  optimize
    (If
       {
         cmp = Eq;
         left = Var (Memory (Const 0));
         right = Sub (Const 1, Const 1);
       })
  |> print;
  [%expect
    {| (If ((cmp Eq) (left (Var (Memory (Const 0)))) (right (Const 0)))) |}]
...
let%expect_test "if optimizes eq when args are const" =
  optimize (If { cmp = Eq; left = Const 0; right = Const 0 }) |> print;
  [%expect {| (Const 1) |}];
  optimize (If { cmp = Eq; left = Const 1; right = Const 0 }) |> print;
  [%expect {| (Const 0) |}]

let%expect_test "if optimizes ne when args are const" =
  optimize (If { cmp = Ne; left = Const 0; right = Const 0 }) |> print;
  [%expect {| (Const 0) |}];
  optimize (If { cmp = Ne; left = Const 1; right = Const 0 }) |> print;
  [%expect {| (Const 1) |}]

let%expect_test "if optimizes lt when args are const" =
  optimize (If { cmp = Lt; left = Const 0; right = Const 0 }) |> print;
  [%expect {| (Const 0) |}];
  optimize (If { cmp = Lt; left = Const 1; right = Const 0 }) |> print;
  [%expect {| (Const 0) |}];
  optimize (If { cmp = Lt; left = Const 0; right = Const 1 }) |> print;
  [%expect {| (Const 1) |}]

let%expect_test "if optimizes le when args are const" =
  optimize (If { cmp = Le; left = Const 0; right = Const 0 }) |> print;
  [%expect {| (Const 1) |}];
  optimize (If { cmp = Le; left = Const 1; right = Const 0 }) |> print;
  [%expect {| (Const 0) |}];
  optimize (If { cmp = Le; left = Const 0; right = Const 1 }) |> print;
  [%expect {| (Const 1) |}]
