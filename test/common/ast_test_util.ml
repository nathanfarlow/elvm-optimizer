...
let print_over_nodes graph ~f ~to_str =
  ...
  |> List.iter ~f:(fun (label, node) -> printf "%s: %s\n" label (f node |> to_str))
;;
...
let graph_with_two_parents () =
  ...
  let graph =
    graph_of_nodes
      [ a_left; a_right; b_left; b_right; jmp_left; jmp_right; putc_a; putc_b; exit ]
  in
  graph, diagram
;;
...
└──────┬──────┘
  ...
  let graph = graph_of_nodes [ a_init; b_init; putc_a; putc_b; b_assign; jmp ] in
  graph, diagram
;;
...
let graph_with_conditional_self_loop () =
  ...
  Node.set_branch jmp (Some (Conditional_jump { true_ = putc_a; false_ = putc_b }));
  Node.set_references
    putc_a
  ...
  Node.set_branch putc_b (Some (Fallthrough exit));
  Node.set_references exit [ { from = putc_b; type_ = Fallthrough } ];
  let graph = graph_of_nodes [ a_init; putc_a; sub_a; sub_b; sub_c; jmp; putc_b; exit ] in
  graph, diagram
;;
