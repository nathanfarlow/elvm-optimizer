    ...
    let parse_exn s ~labels =
      ...
      | None ->
        (match Register.maybe_parse s with
         | Some r -> Register r
         | None ->
           if Hash_set.mem labels s then Label s else failwith @@ "label not found: " ^ s)
    ;;
  end
  ...
  let parse_exn s labels =
    ...
    | [ ".long"; arg ] ->
      (match maybe_parse_number arg with
       ...
       | None ->
         if Hash_set.mem labels arg
         then Some (Init (Long (Label arg)))
         else failwith @@ "unknown argument for .long: " ^ arg)
    | [ ".string"; arg ] ->
      let inside_quotes = Str.regexp {|"\(.*\)"|} in
      if Str.string_match inside_quotes arg 0
      then (
...
let make_sections statements =
  ...
  let add table key data =
    let deque = Hashtbl.find_or_add table key ~default:(fun () -> Deque.create ()) in
      ...
      (match !current_section with
       | Some (Data sub) ->
         (match value with
          ...
          | String s -> String.iter s ~f:(fun c -> add data sub (Const (Char.to_int c))))
       | _ -> failwith "cannot declare data outside .data")
    | Label label ->
      let length table sub =
        Hashtbl.find_or_add table sub ~default:(fun () -> Deque.create ()) |> Deque.length
      in
      let value =
        match !current_section with
...
let make_program statements =
  ...
  let flatten table =
    ...
    let sorted_subsections =
      Hashtbl.to_alist table |> List.sort ~compare:(fun (a, _) (b, _) -> Int.compare a b)
    in
    let flattened =
  ...
  let segment_labels =
    Hashtbl.map labels ~f:(fun (section, offset) ->
      match section with
      | Data subsection -> make_address data_subsection_offsets Data ~subsection ~offset
      | Text subsection -> make_address text_subsection_offsets Text ~subsection ~offset)
  in
  ...
  Hashtbl.add_exn segment_labels ~key:"_edata" ~data:{ segment = Data; offset = data_len };
  Hashtbl.add_exn
    segment_labels
    ~key:const_heap_start_label
...
let filter_noops lines =
  let is_noop line =
    ...
    String.is_empty line || is_prefix "#" || is_prefix ".file" || is_prefix ".loc"
  in
  List.map lines ~f:String.strip |> List.filter ~f:(fun line -> not @@ is_noop line)
;;
